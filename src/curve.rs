...
use bellpepper_core::{ConstraintSystem, SynthesisError, boolean::{Boolean, AllocatedBit}};
use ff::PrimeField;
...
impl<F: PrimeField> AllocatedAffinePoint<F> {
    ...
    {   
        ...
        let dy = AllocatedNum::alloc(
            &mut cs.namespace(|| "alloc dy"), 
        || {
            if is_x_eq.get_value().ok_or(SynthesisError::AssignmentMissing)? {
                Ok(F::ZERO)
            } else {
                Ok(
                    q.y.get_value().ok_or(SynthesisError::AssignmentMissing)? - p.y.get_value().ok_or(SynthesisError::AssignmentMissing)?
                )
            }
        })?;
        cs.enforce(
        ...
        let out_ay = AllocatedNum::alloc(&mut cs.namespace(|| "output ay"), || {
            let mut other_tmp = p.x.get_value().ok_or(SynthesisError::AssignmentMissing)?;
            other_tmp.sub_assign(out_ax.get_value().ok_or(SynthesisError::AssignmentMissing)?);
            let mut tmp = lambda
                .get_value()
                .ok_or(SynthesisError::AssignmentMissing)?;
            tmp.mul_assign(other_tmp);
        ...
        let add_y_is_zero = Boolean::from(AllocatedBit::alloc(
            ...
            {
                Some(p.y.get_value().ok_or(SynthesisError::AssignmentMissing)? + q.y.get_value().ok_or(SynthesisError::AssignmentMissing)? == F::ZERO)
            }
        )?);
        cs.enforce(
